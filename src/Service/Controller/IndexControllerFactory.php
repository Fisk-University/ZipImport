<?php
namespace ZipImport\Service\Controller;

use ZipImport\Controller\IndexController;
use ZipImport\Controller\UnsupportedController;
    ...
    public function __invoke(ContainerInterface $serviceLocator, $requestedName, array $options = null)
    {
        ...
        if (!$this->csvImportIsInstalled()) {
            return new UnsupportedController(
                'ZipImport requires the CSVImport Omeka module to function.'
                . ' Please install it.');
        }

        if (!extension_loaded('zip')) {
            return new UnsupportedController(
                'ZipImport requires the PHP zip extension to function.'
